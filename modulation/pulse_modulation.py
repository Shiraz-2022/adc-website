...
def PPM(inputs):
    def generate_ppm(input_signal, ppm_ratio, pulse_frequency, time_duration):
        ...
        normalized_signal = (input_signal - np.min(input_signal)) / (
            np.max(input_signal) - np.min(input_signal)
        )
        pulse_positions = np.floor(ppm_ratio * normalized_signal * len(t)).astype(int)
...
def PCM(inputs):
    ...
    # Calculate the step size between quantization levels
    step_size = (amplitude_range[1] - amplitude_range[0]) / (
        num_quantization_levels - 1
    )

    # Quantize the analog signal
    quantized_signal = (
        np.round((message - amplitude_range[0]) / step_size) * step_size
        + amplitude_range[0]
    )
    quantized_value = ""
    ...
    encoded_signal = np.array(
        [
            format(int((value - amplitude_range[0]) / step_size), "0{0}b".format(3))
            for value in quantized_signal
        ]
    )
    encoded_str = "".join(encoded_signal)

    def generate_pulse_from_encoded(
        encoded_str, pulse_width, sampling_rate, start_index, num_bits
    ):
        pulse_signal = np.zeros(int(num_bits * sampling_rate * pulse_width))
        encoded_bit = ""
        for i in range(num_bits):
            ...
            if bit == "1":
                pulse_signal[
                    i
                    * int(sampling_rate * pulse_width) : (i + 1)
                    * int(sampling_rate * pulse_width)
                ] = 1
    ...
    encoded_bit, pulse_signal = generate_pulse_from_encoded(
        encoded_str, pulse_width, sampling_rate, start_index, num_bits
    )
    ...
    b = plot_graph(t, quantized_signal, color="green", title="Quantized wave")
    c = plot_graph(
        np.linspace(0, num_bits, len(pulse_signal)),
        pulse_signal,
        color="pink",
        title="pulse",
    )
...
def PWM(inputs):
    ...
    b = plot_graph(t, pwm_signal, title="PWM Signal", condition="plot", color="green")
    c = plot_graph(
        x, modulated_wave, title="Modulated wave", condition="plot", color="blue"
    )
    # d = plot_graph(x, demodulated_wave, title="Demodulated wave",condition="plot",color="blue")
...
def QUANTIZATION(inputs):
    ...
    a = plot_graph(x, message, title="Message", condition="plot", color="red")
    b = plot_graph(
        x, quantized_wave, title="Quantized wave", condition="plot", color="blue"
    )
